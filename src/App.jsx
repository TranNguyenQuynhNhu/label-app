                  ...
                  <div className="max-w-3xl mx-auto">
                    ...
                    <div className="space-y-3">
                      {currentItem.options && currentItem.options.map((opt) => (
                        <div key={opt.id} className={`p-4 rounded-lg border-2 flex items-start gap-4 ${currentItem.answer === opt.id ? 'border-green-400 bg-green-50 shadow-sm' : 'border-slate-200 bg-white'}`}>
                          <span className={`font-bold flex-shrink-0 mt-0.5 ${currentItem.answer === opt.id ? 'text-green-600' : 'text-slate-400'}`}>{opt.id}.</span>
                          <span className="text-slate-700 leading-relaxed">{opt.text}</span>
                        </div>
                      ))}
                    </div>
                  </div>
